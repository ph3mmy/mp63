...
class MainActivity_old : AppCompatActivity() {
    ...
    private val TAG = "MainActivity_old"
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        serverParams = if(parameterResponseString != "" ) {
            ...
            ParameterResponse(mkObj.getString("status"), mkObj.getString("CA"), mkObj.getString("CAL"), mkObj.getString("CC"), mkObj.getString("MT"))
        } else {
            ParameterResponse("", "", "", "", "")
        }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            if (this.checkSelfPermission(Manifest.permission.ACCESS_COARSE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
                if (ContextCompat.checkSelfPermission(this@MainActivity_old,
                        Manifest.permission.ACCESS_FINE_LOCATION) !==
                    PackageManager.PERMISSION_GRANTED) {
                    if (ActivityCompat.shouldShowRequestPermissionRationale(this@MainActivity_old,
                            Manifest.permission.ACCESS_FINE_LOCATION)) {
        ...
        //Initialize the MPOS Device SDK
    ...
    private val bcr = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            // TODO Auto-generated method stub
            try {
                Log.e(TAG, "From the scanner")
                if (BluetoothDevice.ACTION_FOUND == intent.action) {
                    val btDevice =
                        intent.getParcelableExtra<BluetoothDevice>(BluetoothDevice.EXTRA_DEVICE)
                    if (btDevice != null) {
                        mAdapter.addItem(Device(btDevice.name, btDevice.address, false))
                        mAdapter.notifyDataSetChanged()
                    }
                }
            } catch (e: Exception) {
    ...
    private fun downloadKeysFromServer() {
        GlobalScope.launch(Dispatchers.IO) {
            try {
                val masterKeyx = serverRepository.getMasterKey()
                val sessionKeyx = serverRepository.getSessionKey()
                val pinKeyx = serverRepository.getPinKey()

                withContext(Dispatchers.Main) {
                    Log.e("From Main", masterKeyx.toString())
                    Log.e("From Main", sessionKeyx.toString())
                    Log.e("From Main", pinKeyx.toString())

                    if(sessionKeyx!!.status == "00" && pinKeyx!!.status == "00") {

                        val n_esk = ServerUtil.calculateESK(sessionKeyx!!, masterKeyx!!)
                        val n_epk = ServerUtil.calculateEPK(pinKeyx!!, masterKeyx!!)

                        masterKey = masterKeyx
    ...
    private fun downloadParamsFromServer() {
        if(esk != "" && skcv != "") {
            GlobalScope.launch(Dispatchers.IO) {
                try {
                    val parameter = serverRepository.getParameter(Constants.TerminalID, esk, skcv)

                    withContext(Dispatchers.Main) {
                        Log.e("From Main Parameter", parameter.toString())
                        if (parameter != null) {
    ...
    private fun sendTransactionToServer(card: Card) {
        ...
        SweetDialogUtils.showProgress(this@MainActivity_old, "Sending Transaction...", false)
        GlobalScope.launch(Dispatchers.IO) {
            try {
                ...
                val result = serverRepository.sendTransactionData(
                    "http://207.154.228.9:5001/api/v1/pos/process-request/",
                    card
                )
                ...
                withContext(Dispatchers.Main) {
                    ...
                    if(result == null) {
                        SweetDialogUtils.changeAlertType(this@MainActivity_old, "Error: "+result.toString(), SweetAlertDialog.ERROR_TYPE)
                ...
                SweetDialogUtils.changeAlertType(this@MainActivity_old, "Error: $e", SweetAlertDialog.ERROR_TYPE)
    ...
    private fun startDiscovery() {
        if(bluetoothAdapter != null) {
            ...
            try {
                ...
                this@MainActivity_old.registerReceiver(receiver, filter)
                bluetoothAdapter?.startDiscovery()
            } catch (e: Exception) {
                Log.e(TAG, "Error registering bluetooth broadcast receiver.")
            }
            //this@MainActivity_old.unregisterReceiver(receiver)
            mAdapter.notifyDataSetChanged()
        } else {
    ...
    private fun connectDevice(device: Device) {
        SweetDialogUtils.showProgress(this@MainActivity_old, "Connecting...", false)
        GlobalScope.launch {
            if (Controler.posConnected()) {
            ...
            runOnUiThread {
                ...
                Toast.makeText(this@MainActivity_old, "Device Connected", Toast.LENGTH_SHORT)
            }
        }
    }
    ...
    private fun loadAllKeysToDevice() {
        ...
        val keyType = 0; //plain ipek
        val keyIndex = 0; //set by me it can be a value between 0 - 9
        ...
        SweetDialogUtils.showSuccess(this@MainActivity_old, sb.toString())
...
//        SweetDialogUtils.showSuccess(this@MainActivity_old, result.loadResult.toString())
...
//        SweetDialogUtils.showSuccess(this@MainActivity_old, resulty.loadResult.toString())
...
//
//        SweetDialogUtils.showSuccess(this@MainActivity_old, resultx.loadResult.toString())
    }
    ...
    private fun pay() {
        ...
        if(redFlag) {
            SweetDialogUtils.showError(this, "Parameters from Server are missing. Please click on Download Parameters or restart your app.")
            return
        }
        ...
        GlobalScope.launch {
            ...
            if (Controler.posConnected()) {
                ...
                SweetDialogUtils.showError(this@MainActivity_old, "No Device connected.")
            }
            ...
            runOnUiThread {
                ...
                if (cardResult?.cardType == 0) {
                    ...
                    SweetDialogUtils.showError(this@MainActivity_old, errorMessage)
...
//        sb.append("\n\n\npan:" + ret.pan).append("\n");
//        sb.append("pansn:" + ret.pansn).append("\n");
//        sb.append("pinBlock:" + ret.pinblock).append("\n");
//        sb.append("track2:" + ret.track2).append("\n");
//        sb.append("track3:" + ret.track3).append("\n");
//        sb.append("icData:" + ret.icData).append("\n");
//        sb.append("expData:" + ret.expData).append("\n");
//        sb.append("ksn:" + ret.ksn).append("\n");
//        sb.append("mac_ksn:" + ret.mac_ksn).append("\n");
//        sb.append("mag_ksn:" + ret.mag_ksn).append("\n");
//        sb.append("pin_ksn:" + ret.pin_ksn).append("\n");
//        sb.append("encryptPan:" + ret.encryptPan);
            return sb
        } else {
